...
var pager = {
	...
	pagination : function(url, fnName, totalCount, currentPageStartIndex, showRowCount) {
		...
		ol.append($('<input type="hidden" id="paginationUrl" value="'+url+ '" />'));
		ol.append($('<input type="hidden" id="paginationSize" value="'+rowCount+ '" />'));
	}
};
/**
 * url 페이지 이동
 ...
 * 페이지 이동
 */
function goIdxPage(pageIndex){
	$.ajax({
		url : $('#paginationUrl').val() + "?startIndex="+pageIndex+"&pageSize="+$('#paginationSize').val(),
		type : "POST",
		contentType : "application/x-www-form-urlencoded;charset=utf-8",
		success : function(data) {
			$("#pages").html(data);
